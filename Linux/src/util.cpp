...
Table<Section> util::sectab;
Table<Symbol> util::symtab;
...
void util::writeToFile(std::string fileName)
{
	...
	output += util::sectab.to_string();
	
	for (auto &symbol : util::symtab)
		if (!symbol->isSection())
			output += symbol->to_string() + "\n";

	for(auto &section: util::sectab)
	{
...
uint32_t util::readValue(std::string value, std::shared_ptr<Section> currentSection)
{
	...
	auto symbol = util::symtab.find(value);
	auto ret = 0;
	...
	for (auto& line : instructions)
	{
		...
		for (auto& word : line)
		{
			...
			if (word.find(":") != std::string::npos)
			{
				...
				symbol->setIndex(uint16_t(util::symtab.size())+1);
				util::symtab.insert(symbol);
				continue;
			}
			...
			{
				auto name = word;
				auto section = std::make_shared<Section>(name, util::symtab.size() + 1, value, org);
				org = false;
				value = 0;
				
				util::sectab.insert(section);
	...
	for (auto& line : instructions)
	{
		...
		for (auto& word : line)
		{
			...
			if (word[0] == '.')
			{
				...
				//	std::cout << currentSection->to_string_data() << "\n" << currentSection->to_string_table() << "\n\n";
				auto name = word;
				currentSection = util::sectab.find(word);
				break;
			}
			// Check if definition
			if (Definition::contains(word))
			...
			if (def_flag)
			{
				...
				auto symbol = util::symtab.find(word);
...
void util::updateGlobalFlag(std::vector<std::string>& line)
{
	...
	for (auto &word2 : line)
	{
		if (global)
		{
			auto symbol = util::symtab.find(word2);
